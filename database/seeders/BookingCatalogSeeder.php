...
class BookingCatalogSeeder extends Seeder
{
    /**
    ...
    private function seedComplexes(): void
    {
        ...
        $serviceIds = BookingService::whereIn('name', [
            'Снятие и установка колёс',
            'Демонтаж колёс',
            'Монтаж колёс',
            'Балансировка колёс',
    ...
    private function seedPriceListWorks(): void
    {
        foreach (self::PRICE_LIST as $name => $columns) {
            $service = BookingService::updateOrCreate(
                ['name' => $name],
                ['category' => ServiceCategory::Tire, 'is_active' => true, 'base_price' => $columns['passenger'][0]],
            );

            foreach (self::RADIUS_GROUPS as $groupIndex => $radii) {
                foreach ($radii as $radius) {
                    foreach (CarType::bookable() as $carType) {
                        $column = $carType === CarType::Passenger ? 'passenger' : 'suv';
    ...
    private function seedExtraWorks(): void
    {
        foreach (self::EXTRA_WORKS as [$name, $price]) {
            BookingService::updateOrCreate(
                ['name' => $name],
                ['category' => ServiceCategory::Tire, 'is_active' => true, 'base_price' => $price],
            );
        }
    }
    ...
    private function deactivateRemovedServices(): void
    {
        ...
        $removedIds = BookingService::whereNotIn('name', $keep)->pluck('id');

        if ($removedIds->isNotEmpty()) {
            PriceRule::whereIn('service_id', $removedIds)->delete();
            BookingService::whereIn('id', $removedIds)->update(['is_active' => false]);
        }
    }
}
