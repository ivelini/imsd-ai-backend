...
class GenerateSlotGridTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $this->seedWeeklySchedule();
        $generator = app(GenerateSlotGrid::class);
    ...
    public function test_keeps_out_of_schedule_slot_with_booking(): void
    {
        ...
        $user = User::factory()->bookingClient()->create();
        Booking::create([
            'user_id' => $user->id,
            'slot_id' => $slot->id,
            'start_time' => '20:00:00',
        ...
        $this->assertDatabaseHas('booking_slots', ['id' => $slot->id, 'hour' => 20]);
    }
    ...
    public function test_does_not_touch_past_slots(): void
    {
        ...
        $this->assertDatabaseHas('booking_slots', ['id' => $past->id]);
    }
