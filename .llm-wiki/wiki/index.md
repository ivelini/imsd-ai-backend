...
| [Публичный API каталога](concepts/public-catalog-filter-api.md) | Справочник городов (/api/reference/city) + фасеты (/api/reference/filter/tire) + листинг (/api/catalog/tires): контракт фильтров, учёт города, meta.seo, euro_label, агрегация цены, кеш с индексом ключей и JSON-roundtrip (ADR 0004) | 2026-08-21 |
| [Импорт каталога из XLSX](concepts/xlsx-import-pipeline.md) | Пайплайн upload → ImportMasterJob → чанки → ChunkJob, маппинг XLSX → БД (описания, евро-лейбл), инвалидация | 2026-08-21 |
| [Эксплуатация](concepts/operations.md) | Команды Makefile, окружение, очередь и импорты (фактические Job'ы), runbooks | 2026-08-19 |
| [Кеширование](concepts/caching.md) | Cache Service до Action, Observer-инвалидация, TTL, индекс ключей, JSON-roundtrip сериализация (ADR 0004) | 2026-08-21 |
...
| Article | Summary | Updated |
|---------|---------|---------|
| [Шина (TireProduct)](entities/tire-product.md) | tire_products: поля (включая euro_label), сезоны, индексы, парсинг размеров, связи, формула slug | 2026-08-21 |
