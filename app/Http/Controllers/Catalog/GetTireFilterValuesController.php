...
final readonly class GetTireFilterValuesController
{
    ...
    /**
     * Все значения фильтра шин для города по умолчанию.
     */
    #[Response(type: 'array{data: array{width: list<array{label: int, value: int}>, profile: list<array{label: int, value: int}>, diameter: list<array{label: string, value: string}>, season: list<array{label: string, value: string}>, studded: list<array{label: string, value: bool}>, brand: list<array{label: string, value: string}>, country: list<array{label: string, value: string}>, delivery: list<array{label: string, value: string}>, price: array{min: float, max: float}}}')]
    public function __invoke(): TireFilterValuesResource
    {
        // Precondition — только при промахе кеша, перед Action (hit → Action не вызывается)
        $data = $this->tireFilterCache->remember(function (): array {
            $city = $this->ensureCityExists->ensure($this->defaultCityName);

            return $this->getTireFilterValues->execute($city->id);
        });
