...
**Примечания:**
- Запись — снимок параметров и цены на момент создания (ADR 0017); правка прайса задним числом записи не меняет.
- Договор хранения (`storage_*`, см. ниже) — самостоятельная сущность на клиента и с записью не связан.
...
## Домен: Storage (хранение колёс)

Префикс `storage_` отделяет таблицы домена; договор заводится на клиента (`users`) и с записью на шиномонтаж не связан. Деньги — копейки через каст `MoneyCast` (VO `Money`, ADR 0013).
...
**Примечания:**
- Заводит и закрывает договор оператор в панели (кластер «Шиномонтаж», группа «Хранение»); стоимость вводится руками и хранится снимком — расчёт по прайсу «Хранение» не подключён.
- Позиции правятся повторителем в форме договора; состав — минимум одна строка.
